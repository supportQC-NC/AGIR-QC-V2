...
// caractéristiques dynamiques et lien web optionnel (QR code)
// ─────────────────────────────────────────────────────────────────────────────
const PrintModal = ({ isOpen, onClose, onGenerate, articleDesign, nart, article }) => {
  ...
  // Options vitrine — tout activé par défaut sauf marges/stocks
  const [vitrineOptions, setVitrineOptions] = useState({
    photo: true,
    prixHT: false,
    prixTTC: true,
    tgc: true,
    promo: true,
    fournisseur: false,
    refFournisseur: false,
    groupe: false,
    unite: false,
    stocks: false,
  });

  const toggleVitrineOption = (key) => {
    setVitrineOptions((prev) => ({ ...prev, [key]: !prev[key] }));
  };
  ...
  const templates = [
    {
      id: "complete",
      label: "Fiche complète",
      desc: "Toutes les informations : prix, marges, stocks, détails complets",
      icon: "📋",
    },
    {
      id: "vitrine",
      label: "Fiche vitrine",
      desc: "Version client personnalisable — choisissez les infos à afficher",
      icon: "🏪",
    },
  ];
  ...
  const vitrineFields = [
    ...
    { key: "tgc", label: "TGC (ATVA)" },
    { key: "promo", label: "Promo (si active)" },
    { key: "fournisseur", label: "Fournisseur" },
    { key: "refFournisseur", label: "Réf. fournisseur" },
    { key: "groupe", label: "Groupe / Famille" },
    { key: "unite", label: "Unité" },
    { key: "stocks", label: "Stock total" },
  ];
    ...
    <div className="print-modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) handleClose(); }}>
      <div className="print-modal">
        ...
        <div className="print-modal-body">
          ...
          {/* Vitrine options toggles */}
          {template === "vitrine" && (
            <div className="print-section">
              <label className="print-section-label">Informations à afficher</label>
              <span className="print-section-hint-block">NART, Désignation et Code-barres sont toujours inclus</span>
              <div className="vitrine-toggles">
                {vitrineFields.map((f) => (
                  <button
                    key={f.key}
                    className={`vitrine-toggle ${vitrineOptions[f.key] ? "active" : ""}`}
                    onClick={() => toggleVitrineOption(f.key)}
                  >
                    {vitrineOptions[f.key] ? <HiCheckCircle className="toggle-icon on" /> : <HiXCircle className="toggle-icon off" />}
                    <span>{f.label}</span>
                  </button>
                ))}
              </div>
          ...
          <div className="print-section">
            ...
            <div className="chars-list">
              {characteristics.map((c) => (
                <div key={c.id} className="char-row">
                  <input type="text" className="char-input label" placeholder="Label (ex: Poids)" value={c.label} onChange={(e) => handleCharChange(c.id, "label", e.target.value)} />
                  <input type="text" className="char-input value" placeholder="Valeur (ex: 2.5 kg)" value={c.value} onChange={(e) => handleCharChange(c.id, "value", e.target.value)} />
                  <button className="char-remove" onClick={() => handleRemoveCharacteristic(c.id)} title="Supprimer"><HiTrash /></button>
                </div>
              ))}
              <button className="char-add" onClick={handleAddCharacteristic}>
                <HiPlus /> Ajouter une caractéristique
          ...
          <div className="print-section">
            ...
            <input type="url" className="print-input" value={webUrl} onChange={(e) => setWebUrl(e.target.value)} placeholder="https://www.exemple.com/article/..." />
          </div>
        </div>

        <div className="print-modal-footer">
          <button className="print-btn cancel" onClick={handleClose} disabled={generating}>Annuler</button>
          <button className="print-btn generate" onClick={handleGenerate} disabled={generating}>
            {generating ? (<><div className="btn-spinner" /> Génération...</>) : (<><HiPrinter /> Générer le PDF</>)}
          </button>
        </div>
      </div>
    </div>
...
const AdminArticleInfosScreen = () => {
  ...
  const handleGeneratePdf = useCallback(async (options) => {
    if (!article) return;
    const { template, description, characteristics, webUrl, vitrineOptions } = options;

    const gencod = safeTrim(article.GENCOD);
    const barcodeImg = gencod ? generateBarcodeBase64(gencod) : null;
    const photoImg = photoUrl ? await loadImageAsBase64(photoUrl) : null;
    const qrImg = webUrl ? await generateQRCodeBase64(webUrl) : null;
    ...
    //  PHOTO + BLOC PRIX
    // ════════════════════════════════════════════════════════════════
    const showPhoto = !isVitrine || vo.photo;
    const photoBoxW = showPhoto ? 75 : 0;
    const photoBoxH = 65;
    const priceBoxX = showPhoto ? margin + photoBoxW + 8 : margin;
    const priceBoxW = showPhoto ? pb.contentWidth - photoBoxW - 8 : pb.contentWidth;
    ...
    if (showPhoto) {
      ...
      doc.roundedRect(margin, pb.y, photoBoxW, photoBoxH, 2, 2, "FD");
      if (photoImg) {
        try { doc.addImage(photoImg, "JPEG", margin + 2, pb.y + 2, photoBoxW - 4, photoBoxH - 4); }
        catch { doc.setTextColor(...PDF_COLORS.GRAY); doc.setFontSize(10); doc.text("Photo indisponible", margin + 12, pb.y + 35); }
      } else {
        doc.setTextColor(...PDF_COLORS.GRAY); doc.setFontSize(10); doc.text("Photo indisponible", margin + 12, pb.y + 35);
      }
    }
    ...
    const drawPriceLine = (label, value, opts = {}) => {
      const { bold = false, accent = false, strikethrough = false } = opts;
      doc.setFontSize(9); doc.setTextColor(...PDF_COLORS.GRAY_DARK); doc.setFont("helvetica", "normal");
      doc.text(label, priceBoxX + 8, priceY);
      doc.setFontSize(bold ? 15 : 12);
      doc.setTextColor(...(accent ? PDF_COLORS.YELLOW : PDF_COLORS.BLACK));
      doc.setFont("helvetica", bold ? "bold" : "normal");
      ...
      if (strikethrough) {
        const tw = doc.getTextWidth(value);
        doc.setDrawColor(...PDF_COLORS.PROMO_RED); doc.setLineWidth(0.5);
        doc.line(priceBoxX + priceBoxW - 8 - tw, priceY - 1.5, priceBoxX + priceBoxW - 8, priceY - 1.5);
      }
      priceY += 12;
    };

    // Prix — fiche complète : tout ; vitrine : selon toggles
    if (!isVitrine) {
      drawPriceLine("Prix HT (PVTE)", formatPrice(article.PVTE));
      drawPriceLine("TGC (ATVA)", formatPercent(article.ATVA));
    } else {
      if (vo.prixHT) drawPriceLine("Prix HT", formatPrice(article.PVTE));
      if (vo.tgc) drawPriceLine("TGC", formatPercent(article.ATVA));
    }

    if (!isVitrine || vo.prixTTC) {
      drawPriceLine("Prix TTC", formatPrice(article.PVTETTC), { bold: true, strikethrough: promoActive });
    }

    if (promoActive && (!isVitrine || vo.promo)) {
      drawPriceLine(`PROMO -${discount}%`, formatPrice(article.PVPROMO), { bold: true, accent: true });
    }

    // QR code dans le bloc prix
    if (qrImg) {
      const qrSize = 22;
      const qrX = priceBoxX + priceBoxW - qrSize - 5;
      const qrY2 = pb.y + photoBoxH - qrSize - 3;
      doc.addImage(qrImg, "PNG", qrX, qrY2, qrSize, qrSize);
      doc.setFontSize(5); doc.setTextColor(...PDF_COLORS.GRAY);
      doc.text("Voir en ligne", qrX + qrSize / 2, qrY2 + qrSize + 3, { align: "center" });
    }
    ...
    if (promoActive && (!isVitrine || vo.promo)) {
      ...
      doc.setFillColor(...PDF_COLORS.YELLOW);
      doc.roundedRect(margin, pb.y, pb.contentWidth, 16, 2, 2, "F");
      doc.setTextColor(...PDF_COLORS.BLACK); doc.setFontSize(11); doc.setFont("helvetica", "bold");
      doc.text(`PROMOTION  -${discount}%`, margin + 8, pb.y + 7);
      doc.setFontSize(14);
      doc.text(formatPrice(article.PVPROMO), pb.pageWidth - margin - 8, pb.y + 8, { align: "right" });
      doc.setFontSize(7); doc.setFont("helvetica", "italic"); doc.setTextColor(...PDF_COLORS.GRAY_DARK);
      doc.text(`Du ${formatDate(article.DPROMOD)} au ${formatDate(article.DPROMOF)}`, margin + 8, pb.y + 13);
      pb.y += 22;
    }
    ...
    const drawSectionTitle = (title) => {
      ...
      doc.setFillColor(...PDF_COLORS.BLACK);
      doc.roundedRect(margin, pb.y, pb.contentWidth, 8, 1, 1, "F");
      doc.setTextColor(...PDF_COLORS.YELLOW); doc.setFontSize(9); doc.setFont("helvetica", "bold");
      doc.text(title, margin + 4, pb.y + 5.5);
      pb.y += 12;
    };
    ...
    const drawInfoLine = (label, value, opts = {}) => {
      const { indent = 0 } = opts;
      pb.ensureSpace(6);
      doc.setFontSize(8); doc.setFont("helvetica", "normal"); doc.setTextColor(...PDF_COLORS.GRAY_DARK);
      doc.text(label, margin + 4 + indent, pb.y);
      doc.setTextColor(...PDF_COLORS.BLACK); doc.setFont("helvetica", "bold");
      doc.text(String(value || "-"), pb.pageWidth - margin - 4, pb.y, { align: "right" });
      pb.y += 6;
    };
    ...
    // ════════════════════════════════════════════════════════════════
    const infoLines = [];
    if (!isVitrine || vo.fournisseur) infoLines.push(["Fournisseur", article.FOURN || "-"]);
    if (!isVitrine || vo.refFournisseur) infoLines.push(["Réf. fournisseur", safeTrim(article.REFER) || "-"]);
    if (!isVitrine || vo.groupe) infoLines.push(["Groupe / Famille", safeTrim(article.GROUPE) || "-"]);
    if (!isVitrine || vo.unite) infoLines.push(["Unité", safeTrim(article.UNITE) || "-"]);
    if (!isVitrine) infoLines.push(["Conditionnement", article.CONDITNM || "-"]);

    if (infoLines.length > 0) {
      drawSectionTitle("INFORMATIONS PRODUIT");
      infoLines.forEach(([l, v]) => drawInfoLine(l, v));
      pb.y += 2;
    }

    // ════════════════════════════════════════════════════════════════
    //  PRIX (section détaillée)
    // ════════════════════════════════════════════════════════════════
    if (!isVitrine) {
      drawSectionTitle("PRIX");
      drawInfoLine("Prix HT (PVTE)", formatPrice(article.PVTE));
    ...
    // ════════════════════════════════════════════════════════════════
    //  STOCKS (fiche complète, ou vitrine si activé)
    // ════════════════════════════════════════════════════════════════
    if (!isVitrine || vo.stocks) {
      drawSectionTitle("STOCKS");
      drawInfoLine("Stock total", formatStock(calculateStockTotal(article)));
      if (!isVitrine) {
        drawInfoLine("En commande", formatStock(getEnCommande(article)));
    ...
    if (characteristics && characteristics.length > 0) {
      drawSectionTitle("CARACTÉRISTIQUES");
      characteristics.forEach((c) => drawInfoLine(c.label, c.value));
      pb.y += 2;
    }
    ...
    if (description) {
      ...
      doc.setFontSize(8); doc.setFont("helvetica", "normal"); doc.setTextColor(...PDF_COLORS.GRAY_DARK);
      const descLines = doc.splitTextToSize(description, pb.contentWidth - 8);
      descLines.forEach((line) => { pb.ensureSpace(5); doc.text(line, margin + 4, pb.y); pb.y += 4.5; });
      pb.y += 4;
    }
    ...
    if (qrImg && webUrl) {
      ...
      doc.roundedRect(margin, pb.y, pb.contentWidth, 30, 2, 2, "F");
      doc.addImage(qrImg, "PNG", margin + 4, pb.y + 3, 24, 24);
      doc.setTextColor(...PDF_COLORS.BLACK); doc.setFontSize(9); doc.setFont("helvetica", "bold");
      doc.text("Voir cet article en ligne", margin + 34, pb.y + 12);
      doc.setTextColor(...PDF_COLORS.GRAY); doc.setFontSize(7); doc.setFont("helvetica", "normal");
      const urlTruncated = webUrl.length > 80 ? webUrl.substring(0, 77) + "..." : webUrl;
      doc.text(urlTruncated, margin + 34, pb.y + 19);
      pb.y += 34;
    }
    ...
    pb.finalize();
    const filename = isVitrine ? `Fiche_Vitrine_${safeTrim(article.NART)}.pdf` : `Fiche_Complete_${safeTrim(article.NART)}.pdf`;
    doc.save(filename);
  }, [article, photoUrl, hasActivePromo, selectedEntrepriseData, stockEnJours, totalSales, totalRuptures, averageMonthlySales, salesData, mappingEntrepots]);
